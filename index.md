...
The
[`Modified_Hans_Gibbs()`](https://garthtarr.github.io/BayesianLasso/reference/LassoDistribution.md)
function returns a list with the following components:

- `mBeta`: MCMC samples of the regression coefficients $\mathbf{β}$,
  stored as a matrix with `nsamples` rows and `p` columns.
...
- `mA`, `mB`, `mC`: Matrices containing the MCMC samples of the Lasso
  distribution parameters $A_{j}$, $B_{j}$, and $C_{j}$ for each
  coefficient $\beta_{j}$, where each row corresponds to one MCMC
  iteration and each column to a regression coefficient.
